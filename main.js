...
ipcMain.on("foodIdSent", (evt, result) => {
  ...
  const cartItems = `SELECT DISTINCT foods.product_name, varient.name, varient.price, foods.id FROM foods, varient
  ...
  const addonItems = `SELECT  DISTINCT add_on.add_on_name, add_on.price FROM foods, add_on_assign, add_on
